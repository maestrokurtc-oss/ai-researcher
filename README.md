...
브리핑은 수집한 것의 10% 남짓만 싣는다. 나머지가 어디에도 남지 않으면 기준이 맞는지 알 방법이 없으므로, 점수 5.0 이상의 미선정 항목을 `near-misses/YYYY/MM/{날짜}-{회차}.json` 에 보관한다.

검토대(Artifact 페이지)에서 이것들을 훑고 놓치기 아까운 항목에 표시하면, 표시한 항목의 태그·출처·제목이 `data/interests.json` 으로 모여 다음 회차부터 채점 프롬프트에 실린다. 비슷한 항목의 점수가 올라간다. 관심사 밖이라는 이유로 점수를 낮추지는 않으므로 중요한 소식이 묻히지 않는다.

검토대: https://claude.ai/code/artifact/7000b6ea-aa4d-45c2-9aff-a9f71d2185b4

Actions 와 Artifact 는 서로에게 직접 쓸 수 없어서 Claude 세션이 하루 두 번(10:30, 20:30) 양쪽을 잇는다. 절차는 [SYNC.md](SYNC.md) 에 있고, 수동으로도 같은 스크립트로 할 수 있다.

## 알려진 한계

- 검토대 동기화는 Claude 앱이 열려 있을 때만 돈다. 닫혀 있으면 다음 실행 때 밀려서 처리된다.
